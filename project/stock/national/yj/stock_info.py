...
class Stock_Info:
    ...
    def get_information(self):    # 종목의 저점, 고점, 거래량, 지지선, 저항선 구하기
        while self.support == 0:    # 지지선 및 저항선이 구해질 때까지 반복
            ...
            f_volume = open("/workspaces/codespaces-blank/project/stock/national/yj/data/volume"+self.file_name+".txt", "a")    # 거래량을 저장할 파일 열기
...
try:
    api.open_config_file(False)    # 설정 파일 열기
